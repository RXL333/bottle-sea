...
一个能走进去的桌面微缩海洋。Vite + TypeScript strict + Three.js 0.186.0 / WebGL 2，纯前端、无框架、无后端。所有场景模型程序生成；无外部模型、贴图或音频下载。
...
- 声音：用户点击后，启用 Web Audio 合成海风与海浪；再次点击关闭。默认静音。
- 探索模式：从码头进入；WASD 移动，鼠标观察，Space 跳跃/上升，C 下潜，Shift 加速，E 发现。Esc 释放鼠标，点击“返回瓶外”恢复外部视角。
...
- TypeScript strict 检查通过；生产构建通过；22 项测试通过。
