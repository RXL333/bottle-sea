...
export class WeatherSystem extends Group {
  ...
  update(delta:number,time:number){this.intensity+=(Number(this.storm)-this.intensity)*(1-Math.exp(-delta*1.8));this.clouds.visible=this.intensity>.01;this.cloudMaterial.opacity=this.intensity*.92;this.rain.update(time,this.intensity);this.lightning.update(time,this.intensity);}
}
