...
export class WaterCrossing {
  ...
  update(y:number,surface=WATER_LEVEL):'enter'|'leave'|undefined{
    if(!this.initialized){this.initialized=true;this.underwater=y<surface;return;}
    if(!this.underwater&&y<surface-.025){this.underwater=true;return 'enter';}
    if(this.underwater&&y>surface+.025){this.underwater=false;return 'leave';}
  }
}
export class WaterEntrySystem extends Group {
  ...
  update(delta:number,position:Vector3,active:boolean,swimming:boolean,sprint:boolean,surface=WATER_LEVEL){
    this.time+=delta;
    if(active){
      const edge=this.crossing.update(position.y-.38,surface);
      if(edge==='enter'){this.entries++;this.onEnter();for(let i=0;i<this.density;i++)this.emit(position.x,surface,position.z,i/this.density*Math.PI*2,0);}
