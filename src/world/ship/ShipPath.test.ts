...
describe('ship path clearance',()=>{
  it.each([0,1])('keeps the complete calm/storm envelope clear for two circuits',storm=>{
    const period=Math.PI*2/.065,state={height:0,pitch:0,roll:0};
    for(let time=0;time<period*2;time+=period/1440){const pose=sampleShipPath(time);sampleBuoyancy(pose.x,pose.z,pose.yaw,time,storm,state);const c=Math.cos(pose.yaw),s=Math.sin(pose.yaw);
      expect(obbIntersectsAabb(pose.x,pose.z,.34,.6,pose.yaw,DOCK.minX,DOCK.maxX,DOCK.minZ,DOCK.maxZ,.05)).toBe(false);for(const cell of TERRAIN_CELLS)expect(obbIntersectsAabb(pose.x,pose.z,.34,.6,pose.yaw,cell.minX,cell.maxX,cell.minZ,cell.maxZ,.05)).toBe(false);
      for(const [lx,ly,lz] of [[-.34,-.1,-.6],[.34,-.1,-.6],[-.34,.4,.6],[.34,.4,.6],[-.08,1.62,-.55],[.08,1.62,.78]] as const){const x=pose.x+c*lx+s*lz,z=pose.z-s*lx+c*lz,y=state.height+.015+ly;expect(insideBottle(x,y,z,.04)).toBe(true);if(y<4)expect(islandHeight(x,z)).toBe(0);expect(x<DOCK.maxX+.08&&z>DOCK.minZ-.08&&z<DOCK.maxZ+.08).toBe(false);}
    }
  });
